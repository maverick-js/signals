...
import { killNode, isNodeDead, type Node } from './node';
...
export class Scope implements Node {
  ...
  /** @internal */
  _handlers: ErrorHandler<any>[] | null = null;
  /** @internal */
    ...
    if (currentScope) {
      ...
      this._context =
        this._context === defaultContext
          ? currentScope._context
          : { ...currentScope._context, ...this._context };

      if (currentScope._handlers) {
        this._handlers = !this._handlers
          ? currentScope._handlers
 ...
 */
export function appendChild(scope: Scope, child: Node) {
  child._parent = scope;
  child._prev = scope;
